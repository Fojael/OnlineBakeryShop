...
def refund_payment(payment, amount, remarks="Customer refund"):
    """Request a refund for a settled SSLCOMMERZ transaction."""
    ...
    if refund_amount <= Decimal("0.00"):
        raise SSLCommerzError("Refund amount must be greater than zero.")
    ...
    data = {
        ...
        "refund_remarks": _safe_string(remarks, 255),
    ...
    try:
        ...
        raise SSLCommerzError("SSLCommerz refund request timed out.") from exc
    except requests.RequestException as exc:
        raise SSLCommerzError(
            f"Could not connect to SSLCommerz refund API: {exc}"
        ) from exc

    if response.status_code != 200:
        raise SSLCommerzError(
            "SSLCommerz refund API returned HTTP "
    ...
    try:
        ...
        raise SSLCommerzError(
            "SSLCommerz refund API returned invalid JSON."
        ) from exc

    gateway_status = str(payload.get("status", "")).lower()
    if gateway_status not in {"success", "successful"}:
        reason = (
            payload.get("error")
            or payload.get("failedreason")
            or payload.get("message")
            or "SSLCommerz rejected the refund."
        )
        raise SSLCommerzError(str(reason))
